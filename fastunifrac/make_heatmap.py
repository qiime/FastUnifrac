...
def get_info_from_dict(trans_values):
    """
        trans_values: dict of: {(val1, val2): (plot_value, label)}
            must have a key of form (None, None) used for asign value to None values
    ...
    # We need the number of values to know hom many colors we need in the colormap.
...
def make_plot_data(matrix, trans_values):
    """
        ...
        Returns:
            result: list of lists containing the discrete values to plot
                after apply 'trans_values' to 'matrix'
    """
    result = []
...
def plot_heatmap(plot_name, headers, matrix, trans_values, output_dir):
    """
        (This code has benn adapted from Dan Knights' code in make_otu_heatmap.py)
        ...
        Create a heatmap and save it as plot_name.png and plot_name.eps.gz
    ...
    width = (len(headers[HEADERS_HOR]) + 2) / 3 if len(headers[HEADERS_HOR]) > 10 else 10
    height = (len(headers[HEADERS_VER]) + 2) / 3 if len(headers[HEADERS_VER]) > 10 else 10
