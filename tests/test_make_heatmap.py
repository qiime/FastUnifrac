...
from fastunifrac.make_heatmap import get_info_from_dict, get_matrix_value, make_plot_data, plot_heatmap, HEADERS_VER, HEADERS_HOR
...
class MakeHeatmapTest(TestCase):
    def setUp(self):
        ...
        self.headers_ns = {}
        self.headers_ns[HEADERS_HOR] = ["Sample1", "Sample2", "Sample3", "Sample4"]
    ...
    def test_get_info_from_dict(self):
        n_values, boundaries, ticks, ticklabels = get_info_from_dict(self.trans_values)
    ...
    def test_make_plot_data(self):
        ...
        self.assertEqual(make_plot_data(self.matrix, self.trans_values), result_matrix)

        result_matrix = [[0, 1, 4, 2],
                            [0, 0, 4, 3],
                            [0, 0, 0, 2]]

        self.assertEqual(make_plot_data(self.matrix_ns, self.trans_values), result_matrix)
    ...
    def test_plot_heatmap(self):
        ...
        width, height, plot = plot_heatmap(self.plot_name, self.headers, self.matrix, self.trans_values, self.output_dir)

        self.assertEqual(width, 10)
        self.assertEqual(height, 10)
        self.assertTrue(path.exists(png_img_fp), 'The png file was not created in the appropiate location')
        self.assertTrue(path.exists(eps_gz_fp), 'The eps file was not created in the appropiate location')


        png_img_fp_ns = path.join(self.output_dir_ns, self.plot_name_ns + '.png')
        eps_gz_fp_ns = path.join(self.output_dir_ns, self.plot_name_ns + '.eps.gz')
        ...
        self.assertEqual(width, 10)
        self.assertEqual(height, 10)
        self.assertTrue(path.exists(png_img_fp_ns), 'The png file was not created in the appropiate location')
        self.assertTrue(path.exists(eps_gz_fp_ns), 'The eps file was not created in the appropiate location')
