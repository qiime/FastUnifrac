...
from fastunifrac.newick_to_asciiArt import (asciiArt_length, get_tree_by_length_string, add_interactive_sample_id,
...
class NewickToAsciiArtTest(TestCase):
    def setUp(self):
        ...
        self.jack_tree_scaled.scaleBranchLengths(max_length=100, ultrametric=True)
        ...
        dict_mapping_data = {}
        dict_mapping_data["s1"] = {'Description':'s1 test description',
                                    'NumIndividuals':'100',
                                    'BarcodeSequence':'AAAAAAAAAACT',
                                    'LinkerPrimerSequence':'AAAAAAAAAAAAAAAAAAAAA',
                                    'ExampleHeader1':'Value1',
                                    'ExampleHeader2':'Val2'}
        dict_mapping_data["s2"] = {'Description':'s2 test description',
                                    'NumIndividuals':'200',
                                    'BarcodeSequence':'CAAAAAAAAACT',
                                    'LinkerPrimerSequence':'AAAAAAAAAAAAAAAAAAAAA',
                                    'ExampleHeader1':'Value2',
                                    'ExampleHeader2':'Val1'}
        dict_mapping_data["s3"] = {'Description':'s3 test description',
                                    'NumIndividuals':'300',
        ...
        self.mapping_data = [dict_mapping_data, "Example comment string for test"]
    ...
    def test_get_formated_char_html(self):
        c = '/'
        f = 0.4
        obs_string = get_formated_char_html(c, self.num_trees_considered, f, self.trans_values)
        exp_string = """<a href="#" onmouseover="return overlib('<b>Jackknife Count:</b> 4.000<br><b>Jackknife Fraction:</b> 0.400');" onmouseout="return nd();"><font style="BACKGROUND-COLOR:#dddddd">/</font></a>"""
        self.assertEqual(obs_string, exp_string)

        c = '|'
        f = 0.8
        obs_string = get_formated_char_html(c, self.num_trees_considered, f, self.trans_values)
        exp_string = """<a href="#" onmouseover="return overlib('<b>Jackknife Count:</b> 8.000<br><b>Jackknife Fraction:</b> 0.800');" onmouseout="return nd();"><font style="BACKGROUND-COLOR:#82FF8B">|</font></a>"""
        self.assertEqual(obs_string, exp_string)

        c = '\\'
        f = 1.0
        obs_string = get_formated_char_html(c, self.num_trees_considered, f, self.trans_values)
        exp_string = """<a href="#" onmouseover="return overlib('<b>Jackknife Count:</b> 10.000<br><b>Jackknife Fraction:</b> 1.000');" onmouseout="return nd();"><font style="BACKGROUND-COLOR:#FF8582">\\</font></a>"""
        self.assertEqual(obs_string, exp_string)
    ...
    def test_asciiArt_length_html(self):
        obs_jack_lines, obs_mid = asciiArt_length_html(self.jack_tree_scaled, self.num_trees_considered, self.trans_values)
    ...
    def test_draw_jackknife_tree_html(self):
        obs_string = draw_jackknife_tree_html(self.jack_tree, self.num_trees_considered, self.trans_values, self.mapping_data)
    ...
    def test_get_jackknife_tree_html_string(self):
        obs_string = get_jackknife_tree_html_string(self.jack_tree, self.num_trees_considered, self.trans_values, self.mapping_data)

        self.assertEqual(obs_string, exp_html_string)

    def test_make_jackknife_tree_html_file(self):
        self._paths_to_clean_up = [self.output_file, path.join(self.tmp_dir, 'overlib.js')]
        make_jackknife_tree_html_file(self.jack_tree, self.support, self.trans_values, self.mapping_data, self.output_file, self.tmp_dir)

        self.assertTrue(path.exists(self.output_file), 'The html file was not created in the appropiate location')
        self.assertTrue(path.exists(path.join(self.tmp_dir, 'overlib.js')), 'The javascript file was not moved in the appropiate location')
